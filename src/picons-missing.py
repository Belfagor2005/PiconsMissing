...
def missingPicons():
	...
	if os.path.isdir(tmpPicon):
		...
		for ln in piconLocations:
			paths = paths + glob.glob(ln + pattern)
	pathsSplit = []
	for p in paths:
	...
	for message in messages:
		srp_name = message[2][:-4]
		utf8_name = message[7][:-4] if message[7] else ''
		snp_name = message[1][:-4]
		orbital = satname(message[3])
		log.append('"{}","{}",{},"{}",{},"{}",{},{},{}\n'.format(
	...
	for message in messages1:
		...
		log.append('"{}","{}","{}","{}",{}\n'.format(
			clean_name, satname(message[1]), message[2], message[3], message[4]
		))
